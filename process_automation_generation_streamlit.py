...
def run_script(process_name, company_type):
    ...
    save_directory = "./"  # You can change this to your preferred directory
...
# Display the result in the Streamlit interface and provide a link to download the CSV file
    st.write(f"Expert Role: {expert_role}")
    st.write(f"Phases: {phases}")
    st.write(f"Steps: {steps}")
    st.markdown(f"[Download CSV]({csv_file_path})")

# Trigger the function when the button is pressed
if st.button("Generate"):
    run_script(process_name, company_type)